...
const WithdrawPage: React.FC<{ userData: UserProfile | null }> = ({ userData }) => {
  ...
  if (!userData) return <div className="min-h-screen flex items-center justify-center"><Icon name="spinner-third" className="animate-spin text-xl" /></div>;
  ...
  return (
    <div className="max-w-3xl mx-auto px-6 py-10 min-h-screen font-inter bg-zinc-50 dark:bg-[#121212] pb-32">
      <div className="flex items-center space-x-4 mb-8">
        ...
        <h1 className="text-lg font-semibold tracking-tight uppercase text-zinc-900 dark:text-zinc-100">Withdraw Funds</h1>
      </div>
      ...
      <div className="bg-gradient-to-br from-[#06331e] to-emerald-900 text-white p-8 md:p-10 rounded-[2rem] relative overflow-hidden shadow-lg shadow-emerald-900/20 mb-8">
         ...
         <h2 className="text-lg md:text-xl font-semibold tracking-tight relative z-10 truncate">৳{userData.walletBalance || 0}</h2>
      </div>

      <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 p-8 rounded-[2rem] shadow-sm mb-10">
         <div className="flex items-center space-x-3 text-zinc-900 dark:text-zinc-100 font-semibold tracking-tight mb-6">
            <Icon name="money-bill-wave" className="text-emerald-500 text-lg" />
            <span>Request Withdrawal</span>
         </div>
         <div className="space-y-5 mb-8">
            <div>
              <label className="text-[10px] font-bold text-zinc-500 uppercase tracking-normal mb-2 block">Amount (Min ৳{configs?.affiliateMinWithdrawal ?? 50})</label>
              <div className="relative">
                 <div className="absolute left-4 top-1/2 -translate-y-1/2 text-zinc-400 font-bold">৳</div>
                 <input type="number" min={configs?.affiliateMinWithdrawal ?? 50} value={withdrawAmount} onChange={e=>setWithdrawAmount(e.target.value)} placeholder="0" className="w-full bg-zinc-50 dark:bg-zinc-800 border-2 border-zinc-100 dark:border-zinc-700/50 pl-10 pr-4 py-4 rounded-xl outline-none font-semibold text-xl text-zinc-900 dark:text-white focus:border-emerald-500 dark:focus:border-emerald-500 transition-colors" />
              </div>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
               <div>
                 <label className="text-[10px] font-bold text-zinc-500 uppercase tracking-normal mb-2 block">bKash Number</label>
               ...
               <div>
                 <label className="text-[10px] font-bold text-zinc-500 uppercase tracking-normal mb-2 block">Account Name</label>
         ...
         <button onClick={handleWithdraw} disabled={submittingWithdraw} className="w-full bg-[#06331e] text-white py-4 rounded-2xl text-[11px] font-semibold uppercase tracking-normal shadow-lg hover:bg-emerald-950 transition-all active:scale-95 disabled:opacity-50 disabled:active:scale-100 flex items-center justify-center">
      ...
      <div>
        <div className="flex items-center justify-between mb-4 px-2">
          <h3 className="text-sm font-semibold uppercase tracking-normal text-zinc-900 dark:text-zinc-100">Withdrawal History</h3>
          <span className="text-[10px] font-bold text-zinc-400 uppercase tracking-normal">{withdrawals.length} total</span>
        </div>
        ...
        {withdrawals.length === 0 ? (
           <div className="text-center py-16 bg-white dark:bg-zinc-900 rounded-[2rem] border border-dashed border-zinc-200 dark:border-zinc-800">
              <Icon name="history" className="text-xl text-zinc-300 dark:text-zinc-700 mb-4" />
              <p className="text-[10px] font-bold uppercase tracking-normal text-zinc-400">No withdrawals yet</p>
           </div>
        ) : (
                    ...
                    <div className="flex items-center overflow-hidden pr-3">
                       ...
                       <div className="truncate">
                          <div className="flex items-center space-x-2 pb-0.5">
                             ...
                             <span className={`text-[8px] font-semibold px-1.5 py-0.5 rounded-md uppercase tracking-wider ${w.status === 'Completed' ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/40 dark:text-emerald-400' : w.status === 'Rejected' ? 'bg-red-100 text-red-700 dark:bg-red-900/40 dark:text-red-400' : 'bg-yellow-100 text-yellow-700 dark:bg-yellow-900/40 dark:text-yellow-400'}`}>{w.status}</span>
                          </div>
                          <p className="text-[9px] uppercase font-bold text-zinc-400 tracking-normal">{new Date(w.createdAt).toLocaleString()}</p>
                       </div>
                    </div>
                    <div className="shrink-0">
                       <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-700 shadow-[0_2px_8px_rgba(0,0,0,0.04)] text-zinc-900 dark:text-zinc-100 px-3 py-1.5 rounded-full text-[10px] font-semibold uppercase tracking-normal">
                         ৳ {w.amount}
                       </div>
                    </div>
                 </div>
