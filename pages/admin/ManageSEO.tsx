...
const ManageSEO: React.FC = () => {
    ...
    const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>, field: 'faviconUrl' | 'appIconUrl' | 'metaImage') => {
                            ...
                            <div className="flex items-center space-x-4">
                                {seoData.metaImage ? (
                                    <div className="relative group w-32 h-16 shrink-0">
                                       <img src={seoData.metaImage} className="w-full h-full object-cover rounded-lg border border-zinc-200 dark:border-zinc-700 shadow-sm" alt="Meta Image" />
                                       <button type="button" onClick={() => setSeoData({...seoData, metaImage: ''})} className="absolute -top-2 -right-2 bg-red-500 text-white rounded-full p-1 shadow hover:bg-red-600">
                                         <Icon name="times" className="text-[8px]" />
                                       </button>
                                    </div>
                                ) : (
