    ...
    <div className="max-w-6xl mx-auto px-6 py-10 pb-32 min-h-screen bg-zinc-50 dark:bg-zinc-800/50">
       <div className="flex items-center justify-between mb-12">
          <div className="flex items-center space-x-6">
             ...
             <div>
                <h1 className="text-lg md:text-xl font-semibold tracking-tight text-zinc-900 dark:text-zinc-100 mb-1">Coupons & Promos</h1>
                <p className="text-zinc-400 text-[10px] md:text-xs font-bold tracking-normal uppercase">Discount Management</p>
             </div>
          </div>
          <button onClick={() => setShowAddForm(!showAddForm)} className="bg-[#06331e] text-white px-6 py-3 rounded-full text-[10px] font-bold tracking-normal uppercase hover:bg-emerald-900 transition-colors shadow-lg">
       ...
       {showAddForm && (
         <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="bg-zinc-50 dark:bg-zinc-800 p-8 rounded-3xl shadow-sm border border-zinc-200 dark:border-zinc-700 mb-8">
            <h3 className="font-bold text-zinc-900 dark:text-zinc-100 mb-6 uppercase tracking-normal text-xs">Create Discount Code</h3>
            <form onSubmit={handleAddCoupon} className="grid grid-cols-1 md:grid-cols-2 gap-6">
               <div>
                 <label className="block text-[10px] font-bold text-zinc-400 uppercase tracking-normal mb-2">Coupon Code</label>
               ...
               <div>
                 <label className="block text-[10px] font-bold text-zinc-400 uppercase tracking-normal mb-2">Discount Amount</label>
                 <div className="flex space-x-2">
                    ...
                    <select value={newCoupon.type} onChange={e => setNewCoupon({...newCoupon, type: e.target.value})} className="p-4 bg-zinc-50 dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-xl text-sm font-bold uppercase tracking-normal cursor-pointer">
               ...
               <div>
                 <label className="block text-[10px] font-bold text-zinc-400 uppercase tracking-normal mb-2">Max Uses</label>
               ...
               <div className="flex items-end">
                  <button type="submit" className="w-full p-4 bg-emerald-500 text-white rounded-xl font-bold uppercase tracking-normal text-xs hover:bg-emerald-600 transition-colors shadow-lg shadow-emerald-500/20">Create Promo</button>
               </div>
            </form>
         </motion.div>
       )}
         ...
         <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {coupons.map(coupon => (
              <div key={coupon.id} className="bg-zinc-50 dark:bg-zinc-800 p-6 rounded-3xl border border-zinc-200 dark:border-zinc-700 shadow-sm relative group overflow-hidden">
                 ...
                 <div className="flex justify-between items-start mb-6 mt-2">
                    <div className="bg-emerald-50 text-emerald-700 px-4 py-2 rounded-lg font-semibold tracking-normal border border-emerald-100 flex items-center">
                 ...
                 <div className="flex justify-between items-end border-t border-zinc-100 dark:border-zinc-800 pt-4">
                    <div>
                        <p className="text-[10px] font-bold text-zinc-400 uppercase tracking-normal">Discount</p>
                        <p className="text-xl font-semibold text-zinc-900 dark:text-zinc-100">{coupon.type === 'percent' ? `${coupon.discount}%` : `৳${coupon.discount}`}</p>
                    </div>
                    <div className="text-right">
                        <p className="text-[10px] font-bold text-zinc-400 uppercase tracking-normal">Usage</p>
            ...
            {coupons.length === 0 && (
               <div className="col-span-full py-20 text-center bg-zinc-50 dark:bg-zinc-800 rounded-3xl border border-zinc-200 dark:border-zinc-700 shadow-sm text-zinc-400">
                  ...
                  <p className="font-bold text-xs uppercase tracking-normal">No active coupons</p>
               </div>
            )}
         </div>
       )}
